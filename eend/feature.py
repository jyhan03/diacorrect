...
def get_rttm_label(rttm_path, n_spk=2):
    """
    generate rttm label
    
    SPEAKER rec 1   70.10   1.90 <NA> <NA> spk <NA>
    """
    files = open(rttm_path)
    MAX_len = {}
    rttm = {}
    session2spk = {}
    ...
    for line in files:
        ...
        if not spk in rttm[session].keys():
            rttm[session][spk] = np.zeros([MAX_len[session], 1], dtype=np.int32)
            session2spk[session].append(spk)
        start = np.int(np.float(line[3]) * 100)
        end = np.int(np.float(line[4]) * 100) + start
        rttm[session][spk][start: end, -1] = 1
    files.close()
    for session in rttm.keys():
        tmp = np.zeros((MAX_len[session], n_spk))
        rttm[session] = np.concatenate([rttm[session][spk] for spk in session2spk[session]], -1)
        est_spk = rttm[session].shape[-1]
        if est_spk < n_spk:
            tmp[:, :est_spk] = rttm[session]
            rttm[session] = tmp
        else:
            rttm[session] = rttm[session][:, :n_spk]
    return rttm


    
def get_labeledSTFT_rttm(
        kaldi_obj, rttm_obj,
        rec, start, end, frame_size, frame_shift,
        n_speakers=None):
    """ Extracts STFT and corresponding labels
    ...
    Returns:
        ...
        T: label
            (n_frmaes, n_speakers)-shaped np.int32 array.
    rttm_obj:
        [dict, key=rec, val=['utt':, 'spk':, 'st':, 'et':]]
    """   
    data, rate = kaldi_obj.load_wav(
            rec, start * frame_shift, end * frame_shift)
    Y = stft(data, frame_size, frame_shift)
    ...
    # initial RTTM processing
    rttm_segments = rttm_obj[rec]
    rttm_spk = np.unique([seg['spk'] for seg in rttm_segments]).tolist()
    rttm_T = np.zeros((Y.shape[0], n_speakers), dtype=np.int32)    
    ...
    for seg in rttm_segments:
        speaker_index = rttm_spk.index(seg['spk'])
        start_frame = np.rint(
                seg['st'] * rate / frame_shift).astype(int)
        end_frame = np.rint(
                seg['et'] * rate / frame_shift).astype(int)
        ...
        if start < end_frame and end_frame <= end:
            rel_end = end_frame - start
        if rel_start is not None or rel_end is not None:
            rttm_T[rel_start:rel_end, speaker_index] = 1            
    
    return Y, T, rttm_T
  
    
def get_labeledSTFT_sad(
        kaldi_obj,
        rec, start, end, frame_size, frame_shift,
        n_speakers=None,
        use_speaker_id=False):
...
if __name__ == '__main__':
    rttm_path = r'C:\Users\Jyhan\Desktop\喜马拉雅\work\diarization\ustc-ximalaya\ts-vad_ustc\revised\debug\hyp_0.5_11.rttm'
    rttm = get_rttm_label(rttm_path)
    rec = 'data_simu_wav_test_clean_ns2_beta2_500_4_mix_0000500'
    rttm[rec]
#    spk1 = 'data_simu_wav_test_clean_ns2_beta2_500_4_mix_0000500_0'
#    spk2 = 'data_simu_wav_test_clean_ns2_beta2_500_4_mix_0000500_1'
#    a = rttm[rec][spk1]
#    b = rttm[rec][spk2]
#    spk_list = np.concatenate([rttm[rec][spk] for spk in session2spk[rec]], -1)
