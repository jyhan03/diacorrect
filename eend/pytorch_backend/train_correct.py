...
from eend.pytorch_backend.nnet_correct import TransformerCorrection, NoamScheduler
from eend.pytorch_backend.diarization_dataset_correct import KaldiDiarizationDataset, my_collate
...
def train(args):
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        loss_epoch = 0
        num_total = 0
        for step, (y, t, sas) in tqdm(enumerate(train_iter), ncols=100, total=len(train_iter)):
            y = [yi.to(device) for yi in y]
            t = [ti.to(device) for ti in t]
            sas = [si.to(device) for si in sas]
            output = model(y, sas)
